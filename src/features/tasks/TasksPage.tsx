...
export function TasksPage() {
  ...
  const tasks = useRealtimeTable('tasks', 'trip_id', activeTripId, { orderBy: 'due_date' })
  ...
  return (
    <div>
      ...
      {tasks.data.length === 0 ? (
        <EmptyState icon={<CheckSquare className="size-8" />} title="No tasks yet" description="Add pre-trip to-dos grouped by category." />
      ) : (
        <div className="space-y-5">
          {groups.map((group) => {
            ...
            return (
              <div key={group}>
                <h3 className="mb-2 text-sm font-semibold">{GROUP_LABELS[group]}</h3>
                  ...
                  {groupTasks.map((task) => (
                    <div key={task.id} className="flex flex-wrap items-center gap-2 rounded-md border border-border px-2.5 py-2">
                      ...
                      <Select
                        ...
                        onValueChange={(v) => tasks.update(task.id, { assignee_id: v === 'unassigned' ? null : v })}
                      >
                ...
                <div className="mt-1.5 flex gap-2">
                  <Input
                    ...
                    onChange={(e) => setNewTitle((prev) => ({ ...prev, [group]: e.target.value }))}
                    placeholder="Add a task…"
                    className="h-8 text-sm"
                    onKeyDown={async (e) => {
                      if (e.key === 'Enter' && newTitle[group]?.trim()) {
                        await tasks.insert({ trip_id: activeTripId!, group_name: group, title: newTitle[group].trim() })
